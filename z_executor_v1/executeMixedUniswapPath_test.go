...
func TestExecuteMixedPathWithV3(t *testing.T) {
	...
	pathExecutionValues, err := convertPathToPathExecutionValues(profitablePath)
	if err != nil {
		fmt.Println("err: ", err)
		t.Fatal(err)
	}
	...
	quoterFlashswapAmountOut, err := uniswap_v3.GetAmountOutWithQuoter(*V3FlashswapPool, profitablePath.AmountIn, pathExecutionValues.FlashswapZeroForOne, client)
	if err != nil {
		fmt.Println("err: ", err)
		t.Fatal(err)
	}
	...
	for i := 0; i < len(pathExecutionValues.SwapDatas); i++ {
		fmt.Println("amountOuts: ", pathExecutionValues.SwapDatas[i].AmountOut.String())
	}

	fmt.Println("flashswapAmountOut", pathExecutionValues.FlashswapAmountOut.String())

	fmt.Println("revenue: ", pathExecutionValues.Revenue.String())
	fmt.Println("quoterRevenue", quoterRevenue.String())

	//print protocols of each pool
	fmt.Println("flashswapIsV2: ", pathExecutionValues.FlashswapIsV2)
	fmt.Println("isV2s: ", pathExecutionValues.SwapDataIsV2s)
	fmt.Println("calldata: ", hex.EncodeToString(calldata))
	fmt.Println("blockNumber:", header.Number.String())
	fmt.Println("target:", pathExecutionValues.ExecutorAddress.String())
