...
func HandleNewBlock(pools []pool_interface.PoolInterface, poolToPathMap map[common.Address][]path.Path, poolAddressToPoolMap map[common.Address]pool_interface.PoolInterface, config types.Config, header *geth_types.Header) error {
	...
	uniqueAffectedPaths := make([]*path.Path, 0)
	uniqueAffectedPathMap := make(map[*path.Path]bool)
	...
	pools = make([]pool_interface.PoolInterface, 0)
	for _, poolAddress := range v2AffectedAddresses {
...
func handleAffectedPath(path path.Path, header *geth_types.Header, config types.Config) error {
