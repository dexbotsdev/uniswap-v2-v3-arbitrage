...
contract FBFSV3 {
    ...
    function uniswapV2Call(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external
    {
        ...
        assembly {
            ...
            //let amountOutNos := shr(248, calldataload(197)) 
            //start if bytes is 36
            let amount0 := calldataload(36)//amount out 0
            let amount1 := calldataload(68)//amount out 1
            let revenue := calldataload(164)
            let maxSwapIndex := shr(248, calldataload(196))             //swap amounts
            let amountOutNos := shr(248, calldataload(197))            // bytes1 each bit represents if amountOut is 0 or 1 in order
