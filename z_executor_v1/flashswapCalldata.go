...
func createFlashswapCalldata(pathExecutionValues PathExecutionValues, payload []byte) ([]byte, error) {
	...
	if pathExecutionValues.FlashswapIsV2 {
		//TODO
		...
		if pathExecutionValues.FlashswapZeroForOne {
			amountOut0 = big.NewInt(0)
			amountOut1 = new(big.Int).Set(pathExecutionValues.FlashswapAmountOut)
		} else {
			amountOut0 = new(big.Int).Set(pathExecutionValues.FlashswapAmountOut)
		...
		fmt.Println("flashswap zeroForOne", pathExecutionValues.FlashswapZeroForOne)
		fmt.Println("flashswap amountIn", pathExecutionValues.FlashswapAmountIn)
		...
		flashswapCalldata, err := GetUniswapV3ExecutorCalldata(
			pathExecutionValues.FlashswapTarget,
			pathExecutionValues.FlashswapZeroForOne,
			pathExecutionValues.FlashswapAmountIn,
			payload,
		)
		if err != nil {
			return nil, err
