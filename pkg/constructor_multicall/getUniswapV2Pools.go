...
func GetUniswapV2PoolsCall(factoryAddress common.Address, start *big.Int, end *big.Int, client *ethclient.Client) (*big.Int, []types.UniV2Pool, error) {
	...
	pools := make([]types.UniV2Pool, 0)
	for i := 0; i < len(addresses); i++ {
		newPool := types.UniV2Pool{
			Address:        addresses[i],
...
func GetUniswapV2PoolsFromFactory(factoryAddress common.Address, client *ethclient.Client) ([]types.UniV2Pool, error) {
...
func GetUniswapV2PoolsFromAllForks(client *ethclient.Client) ([]types.UniV2Pool, error) {
	...
	factoryAddresses = append(factoryAddresses, croFactoryAddress)
	factoryAddresses = append(factoryAddresses, zeusFactoryAddress)
	factoryAddresses = append(factoryAddresses, luaFactoryAddress)
