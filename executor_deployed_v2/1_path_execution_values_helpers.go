...
func getV2FactoryIndex(factoryAddress common.Address) (int, error) {
	...
	if factoryAddress == common.HexToAddress("0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac") {
		return 1, nil
	}
	if factoryAddress == common.HexToAddress("0x1097053Fd2ea711dad45caCcc45EfF7548fCB362") {
		return 2, nil
	}
	return 0, errors.New("factory not found")
}

func getV3Factory(factoryAddress common.Address) (int, error) {
	//index 1 is not used as it is reserved for a custom address
	...
	if factoryAddress == common.HexToAddress("0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865") {
		return 2, nil
	}
	return 0, errors.New("factory not found")
}
...
func getV3SwapDatas(path path.Path, targets []common.Address, amountsIndexMap map[int]int, tokenToIndexMap map[common.Address]int) ([]V3SwapData, error) {
	...
	for i := 0; i < len(path.Pools); i++ {
		if path.Pools[i].GetType() == "uniswap_v3" {
			...
			factoryIndex, err := getV3Factory(path.Pools[i].GetFactoryAddress())
			if err != nil {
				return nil, err
			}
			amountIndex := amountsIndexMap[i]
