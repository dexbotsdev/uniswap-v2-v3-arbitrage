...
func TestExecuteMixedPathWithV3V2V3(t *testing.T) {
	...
	for i := 0; i < len(paths); i++ {
		...
		if paths[i].Pools[len(paths[i].Pools)-1].GetFactoryAddress().Hex() != "0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac" && paths[i].Pools[0].GetType() == "uniswap_v3" && paths[i].Pools[1].GetType() == "uniswap_v2" && paths[i].Pools[2].GetType() == "uniswap_v3" {
			err = UpdatePools(paths[i].Pools, client)
			if err != nil {
				fmt.Println("err: ", err)
				t.Fatal(err)
